...
//Strength Calculator *new, used to check strength margine
int strength[MAX];
    ...
    for (int i = 0; i != candidate_count; i++) //navigate through preferences array
    {
        for (int j = i + 1; j != candidate_count; j++) //tried at j = 0, get duplicate pairs. Using i+1 skips pairs already found
        {
            ...
            else if (preferences[i][j] > preferences[j][i])
            {
                ...
                strength[pair_count] = preferences[i][j];
                pair_count++;
            }
            else //if (preferences[j][i] < preferences[i][j]) //rather than cycling through whole thing, do binary search at once
            {
                ...
                strength[pair_count] = preferences[j][i];
                pair_count++;
            }
        }
    }
...
void sort_pairs(void)
{
    ...
    for (int i = 0; i != pair_count; i++)
    {
        ...
        {
            if (strength[i] < strength [j]) //checking strength margin
            {
                max = j;
            }
        }
        //Pullin the ol switcharoo, linear search
        tmp_str = strength[max];
        tmp_pair.winner = pairs[max].winner;
        tmp_pair.loser = pairs[max].loser;
        strength[max] = strength[i];
        pairs[max].winner = pairs[i].winner;
        pairs[max].loser = pairs[i].loser;
        strength[i] = tmp_str;
        pairs[i].winner = tmp_pair.winner;
        pairs[i].loser = tmp_pair.loser;
    }
