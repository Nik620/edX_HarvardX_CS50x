...
#include <cs50.h>
#include <stdio.h>
#include <strings.h> //not sure what this does...
...
bool check(const char *word)
{
    ...
    while (cursor != NULL) //cursor -> next != NULL didn't seem to work, last case at NULL
    {
...
unsigned int hash(const char *word)
{
    ...
    //return index
    return index % N;
    //**MAJOR BUG**, found out that this was a cause
    //of most of my segmentation errors. Changed
    //to index % N instead of index to fix.
}
...
bool load(const char *dictionary)
{
    ...
    //**MAJOR BUG** found out that calling on
    //FILE file causes a segmentation error. Changed
    //the name from file to dictionary_file
...
bool unload(void)
{
    ...
    for (int i = 0; i < N; i++)
    {
        node *start = table[i]; //so don't lose place of first
        node *cursor = start;
        while (cursor != NULL)
        {
            node *tmp = cursor;
