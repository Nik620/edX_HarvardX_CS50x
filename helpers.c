...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    int *average = malloc(8); //average counter, playing with pointers
...
void reflect(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    RGBTRIPLE *temp = malloc(24); //temp storage to overwrite & recall values
    ...
    free(temp); //again playing around with pointers
    return;
}
